# 🚀 Script de Test Rapide (Inférence Seule)

Utilisez ce script dans un **nouveau notebook Google Colab vierge** si votre environnement s'est fermé après l'entraînement. Il charge directement vos poids entraînés depuis Google Drive et lance les tests critiques, sans avoir à ré-entraîner quoi que ce soit !

---

## Cellule 1 : Installation et Chargement de l'IA (Depuis Drive)
...
print(f"⏳ Chargement de votre IA experte depuis : {save_path}")

model, tokenizer = FastLanguageModel.from_pretrained(
    model_name = save_path, # On charge vos poids LoRA directement !
    max_seq_length = 2048,
    dtype = None,
    load_in_4bit = True,
)

# Activation du moteur d'inférence (génération ultra-rapide)
FastLanguageModel.for_inference(model)

# --- DÉFINITION DES PROMPTS SYSTÈME ---
prompt_system_sql = (
...
print("✅ Modèle chargé et prêt pour le diagnostic !")
```

---

## Cellule 2 : Le Diagnostic Automatisé (Stress Test)

Copiez ce code dans la deuxième cellule. C'est exactement le même script de test critique que nous avons préparé.
